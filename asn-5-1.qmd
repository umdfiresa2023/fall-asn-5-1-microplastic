...
dfcountyPIC <- finaldf[finaldf$Parameter == "PIC", ]
# Create a summary data table with the total sum of MeasureValue for each location
year_sum_PIC <- aggregate(PIC ~ Year + name + state, data = dfcountyPIC, sum)


# Create a line graph with points for all areas
ggplot(year_sum_PIC, aes(x = Year, y = PIC, group = interaction(name, state), color = interaction(name, state))) +
  geom_line() +
  geom_point() +
  theme_minimal() +
