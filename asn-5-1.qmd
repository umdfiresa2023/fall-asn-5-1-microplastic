...
Your team should not aggregate the data to the year level. Instead, you should have used monthly data to include seasonal variation. (-1)

Also, PIC is not summable. Your team should have used the mean instead. Please see my suggested code below:
...
Please see my suggestion using monthly data:

```{r}
model_score <- lm(PIC ~ Year, data = df)
model_score
```
...
```{r}
model<- felm(PIC ~ Year + Ban + Tax | name + Month , df)
summary(model)
```
